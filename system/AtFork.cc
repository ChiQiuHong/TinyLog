...
namespace tinylog
{
        ...
        std::function<void()> child)
    {
        ...
        {
            throw std::invalid_argument("at-fork: append: duplicate");
        }
        auto task =
    ...
    void AtForkList::remove(void const *handle)
    {
        ...
        if (i1 == index.end())
        {
            throw std::out_of_range("at-fork: remove: missing");
        }
        auto i2 = i1->second;
        index.erase(i1);
        tasks.erase(i2);
    ...
    namespace
    {
            ...
            void (*prepare)(), void (*parent)(), void (*child)())
        {
            ...
            if (ret != 0)
            {
                throw std::system_error(ret, std::generic_category(), "pthread_atfork failed");
            }
        }
        ...
        struct AtForkListSingleton
        {
            ...
            static void child()
            {
                if (!SkipAtForkHandlers::value)
                {
                    ...
                    // while handing the child callbacks
